...
import { loadGoogleMapsApi, type GoogleMapsNamespace } from "./googleMapsLoader";
import type { GeoPoint, GoogleMapMarker } from "./types";
...
export const GoogleMap = ({
  ...
  }, [effectiveCenter.lat, effectiveCenter.lng, options, zoom]);
    ...
    errorMessage != null ? (
      errorFallback ?? (
        <div className="space-y-2">
          ...
          <div className="text-sm text-slate-600">{errorMessage}</div>
        </div>
      )
    ) : markers.length === 0 && !center ? (
      emptyFallback ?? (
        <div className="space-y-2">
          <div className="text-sm font-semibold text-slate-900">No map data available</div>
  ...
  return (
    <div className={cx("relative min-h-[320px] overflow-hidden rounded-2xl", className)} style={style}>
      <div
        ref={containerRef}
        className={cx(
          "h-full w-full min-h-[320px] bg-slate-100",
          isReady ? "opacity-100" : "opacity-70"
        )}
      />

      {overlayContent ? (
        <div className="absolute inset-0 flex items-center justify-center bg-white/70 p-6 text-center backdrop-blur-[2px]">
          {overlayContent}
        </div>
      ) : null}
    </div>
