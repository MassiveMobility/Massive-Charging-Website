...
const EVCarsCatalogue = ({ database }) => {
  ...
  const carList = useMemo(() => {
    if (!database) return [];
    return database.Vehicle_master
      .filter(v => v.Category_ID === "CAT_006")
      .map(v => {
                  ...
                  >
                    ...
                    <div className="space-y-0.5 mb-3">
                      ...
                      <p className="text-sm font-bold text-slate-900">
                        {car.details?.Price || 'Price on Request'}
                      </p>
                    </div>
                    ...
                    <div className="flex items-center gap-3 pt-3 border-t border-slate-200">
                      <div className="flex items-center gap-1 text-slate-500">
                        ...
                        <span className="text-[10px] font-semibold">
                          {car.details?.Battery_Capacity || 'N/A'}
                        </span>
                      </div>
                      <div className="flex items-center gap-1 text-slate-500">
                        <Navigation size={12} />
                        <span className="text-[10px] font-semibold">
                          {car.details?.Claimed_Range || 'N/A'}
                        </span>
                      </div>
                      <ChevronRight 
                        size={14} 
                ...
                <div className="p-6 space-y-5">
                  ...
                  <button 
                    onClick={() => {
                      ...
                      const guideArticle = database.Guide_article?.find(g => g.Guide_ID === displayCar.details?.Guide_ID);
                      if (guideArticle) {
